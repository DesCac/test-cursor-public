...
Если видите `Unexpected item 'parameters › symfony'`:
- Это уже исправлено - добавлены `includes` для расширений
- Контейнер Symfony компилируется перед запуском PHPStan
- Добавлены ignoreErrors для известных предупреждений CI
