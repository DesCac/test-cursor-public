...
## Редизайн редакторов логики

### Требования к UX
- Универсальный каркас для NPC и квестов: верхняя панель действий, левая палитра блоков, центральный холст VueFlow, правый сайдбар свойств.
- Drag-and-drop из палитры, свободное перемещение узлов, создание связей мышью, быстрые стрелки подключения.
- Цветовые схемы и иконки по типам узлов (start/dialog/choice/action/end и start/objective/condition/reward/end), бейджи состояния (условия заданы, переходы не настроены и т.д.).
- Редактирование переходов в сайдбаре: текст выбора, условия, управление несколькими вариантами, быстрые действия (дублировать, удалить).
- Вкладки «Данные», «Условия», «Переходы», валидатор JSON с подсветкой ошибок и пресетами (уровень персонажа, нужный предмет).
- Управление видом: fit-to-view, сетка, мини-карта, всплывающие подсказки, автосохранение позиций, подтверждение при выходе с несохраненными изменениями.

### Внутренние компоненты
- `assets/components/flow/Palette.vue` — список доступных типов узлов с drag source.
- `assets/components/flow/Canvas.vue` — обертка над VueFlow с темизацией, обработчиками событий, подсветкой активных элементов.
- `assets/components/flow/PropertiesPanel.vue` — правый сайдбар с вкладками, формами данных/условий, кнопками переходов.
- `assets/components/flow/EdgeEditor.vue` — отдельный редактор подписей и условий связей (диалоги, ветвления квестов).
- `assets/components/flow/useFlowStore.ts` — композиционный слой/Pinia store для состояния графа, выбранных элементов, истории действий.
- `assets/components/flow/utils/serialization.ts` — функции трансформации между VueFlow элементами и контрактами REST API.

### Данные и сохранение
- Загрузка: нормализация API-ответов, добавление временных ID, установка дефолтных позиций/цветов, заполнение палитры.
- Сохранение: валидация обязательных полей по типу, преобразование условий и данных в объекты, PUT `/api/npcs/{id}/nodes` и новая ручка `/api/quests/{id}/nodes`.
- Обработка ошибок: toast-уведомления, выделение проблемных полей, сохранение черновика локально (например, `localStorage`).
- Поддержка горячих клавиш: Delete для удаления, Cmd/Ctrl+S — сохранить, Cmd/Ctrl+0 — fit-to-view, Cmd/Ctrl+Z/Y — undo/redo (минимум на уровне состояния фронта).

### Вдохновение из n8n
- Закрепленные хедеры с основными действиями, hover-toolbar поверх выделенного узла, контекстное меню по правой кнопке.
- Миникарточки узлов с компактной информацией: заголовок, ключевые поля данных, индикатор условий, количество исходящих переходов.
- Состояния соединений (active, условное, заблокировано) отображаются цветом/стилем линии.

### TODO на будущее
- Обсудить потенциальную миграцию на Rete.js для продвинутых сценариев (сохранить отдельный пункт в backlog).
- Добавить совместное редактирование и live cursors → потребуется backend-синхронизация (WebSocket).
- Интегрировать превью диалога/квеста в реальном времени (симуляция прохождения).
