...
**Причина (ранее):** Symfony Flex пытался обработать минимальный `symfony.lock`, из-за чего возникала ошибка типов.

**Решение в проекте:**
1. В репозитории хранится полный `symfony.lock`, синхронизированный с `composer.json`
2. В CI/CD и локально достаточно повторно выполнить `composer install` — удалять lock-файлы больше не требуется

### Auto-scripts в composer.json

**Изменение:** Возвращены стандартные рецепты (`cache:clear`, `assets:install`)

**Причина:**
- Синхронизируемся с официальными рецептами Symfony Flex
- Обеспечиваем корректное развёртывание фронтенд-активов при установке
- Команды запускаются в CI и локально после установки зависимостей
...
```
Эта последовательность по-прежнему работает, если требуется пропустить auto-scripts в первом шаге.
